#!/bin/bash
  ##############################################################
  # File Name: CentOS6 system check.sh
  # Version: V1.0
  # Author:gaovce
  # Time:2018/11/3
  # Organization: 
 ...
 function getCpuStatus(){ #获取CPU相关信息
     echo ""
...
function getDiskStatus(){
    ...
    inodedata=$(df -iTP | sed '1d' | awk '$2!="tmpfs"{print}')    inodetotal=$(echo "$inodedata" | awk '{total+=$3}END{print total}')
...
function getSystemStatus(){
    ...
    if [ -e /etc/sysconfig/i18n ];then   #获取系统的语言环境
...
function getUserLastLogin(){
    ...
    while(( $thisYear >= $oldesYear));do
        ...
        if [ $loginBeforeToday -eq 0 ];then            echo "从未登录过"
            break
